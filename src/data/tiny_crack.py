...
class TinyCrackDS:
    ...
    def __new__(cls, data_config):
        return (TinyCrack(split=split, **data_config) for split in TinyCrackDS.SPLITS)
    

class TinyCrack(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.img_path = os.path.join(self.base_path, 'imgs')
        self.ann_path = os.path.join(self.base_path, 'anns')
        ...
        if deterministic:
            self.seed = 1744941733
            log.info(f'Deterministic dataset {self.__class__}:split={self.split}, numpy seed set to {self.seed}.')
