...
class CrackSegDS:
    ...
    def __new__(cls, data_config):
        return (CrackSeg(split=split, **data_config) for split in CrackSegDS.SPLITS)
...
class CrackSeg(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ann_path = os.path.join(self.base_path, 'mask')
        ...
        if deterministic:
            self.seed = 1744941733 # int(time.time())
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_img = np.array(Image.open(os.path.join(self.img_path, os.path.basename(query_name[:-3] + 'jpg'))).convert('RGB'))

        support_ids = [os.path.basename(name[:-3] + 'jpg') for name in support_names]
